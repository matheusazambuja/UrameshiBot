...
async def cmd_help(ctx):
    ...
    embed.add_field(
        ...
        inline=False)
    embed.add_field(
        name=f'- Command: ```{prefix}seasons```',
        value="Animes season prequel and sequel",
        inline=False
    )
    embed.set_footer(
        text='Requested by ' + ctx.message.author.name,
...
async def cmd_songs(ctx, *name: str):
    ...
    if anime_found['found']:
        ...
        if anime_found['found']:
            ...
            str_op = '\n'.join(
                [f"Name: {op}" for op in anime_found['op']])
            embed.add_field(
                name=f'-- Openings: --',
                value=f"```{str_op}```",
                inline=False)
            ...
            embed.set_thumbnail(url=anime_found['image_url'])
            str_end = '\n'.join(
                [f"Name: {end}" for end in anime_found['end']])
            embed.add_field(
                name=f'-- Endings: --',
                value=f"```{str_end}```",
                inline=False)
...
async def cmd_schedule(ctx, *day: str):
    ...
    if animes_today['found']:
        ...
        if animes_today['error'] == 'empty':
            await ctx.send(f'```No results for this day```')
        elif animes_today['error'] == 'arg_error':
            await ctx.send('```Argument error: day error```')
        elif animes_today['error'] == 'api_error':
            await ctx.send('```API error```')
...
async def cmd_season(ctx, *name: str):
    ...
    if not anime_found['found']:
        if anime_found['error'] == 'api_error':
            await ctx.send('```API error```')
        elif anime_found['error'] == 'empty':
            await ctx.send('```No results for this anime```')
